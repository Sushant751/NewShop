...
public sealed class ReportRepository : IReportRepository
{
    ...
    public async Task<DashboardSummary> GetDashboardSummaryAsync(DateTime from, DateTime to, CancellationToken cancellationToken = default)
    {
        ...
        const string sql = @"
            SELECT
                ...
                ), 0) AS TotalProfit,  -- reused as COGS temp; replaced below
        ...
        var cogs = summary.TotalProfit; // we stored COGS in TotalProfit column temporarily
        var netProfit = summary.TotalSales - cogs - summary.TotalExpenses;
        return summary with { TotalProfit = netProfit };
    }


    public async Task<IReadOnlyList<TopProductRow>> GetTopProductsAsync(DateTime from, DateTime to, int top = 10, CancellationToken cancellationToken = default)
    {
        var tenantId = RequireTenantId();
        const string sql = @"
            SELECT TOP (@Top)
    ...
    public async Task<IReadOnlyList<DailySalesRow>> GetDailySalesAsync(DateTime from, DateTime to, CancellationToken cancellationToken = default)
    {
        var tenantId = RequireTenantId();
        const string sql = @"
            SELECT
    ...
    public async Task<ProfitLossRow> GetProfitLossAsync(DateTime from, DateTime to, CancellationToken cancellationToken = default)
    {
        var tenantId = RequireTenantId();
        const string sql = @"
            SELECT
    ...
    public async Task<IReadOnlyList<SalesReportRow>> GetSalesReportAsync(DateTime from, DateTime to, CancellationToken cancellationToken = default)
    {
        var tenantId = RequireTenantId();
        const string sql = @"
            SELECT
    ...
    public async Task<IReadOnlyList<GstRateBreakdownRow>> GetGstReportAsync(DateTime from, DateTime to, CancellationToken cancellationToken = default)
    {
        var tenantId = RequireTenantId();
        const string sql = @"
            SELECT
    ...
    public async Task<IReadOnlyList<PaymentMethodSummaryRow>> GetPaymentMethodSummaryAsync(DateTime from, DateTime to, CancellationToken cancellationToken = default)
    {
        var tenantId = RequireTenantId();
        const string sql = @"
            SELECT
    ...
    public async Task<IReadOnlyList<InventoryValuationRow>> GetInventoryValuationAsync(CancellationToken cancellationToken = default)
    {
        var tenantId = RequireTenantId();
        const string sql = @"
            SELECT
